...
class MessageService {
  ...
  async deleteMessage(messageId: string, discussionId: string): Promise<MessageDto | null> {
    ...
    const userMessage: HydratedDocument<Message> | null = await MessageModel.findByIdAndDelete(messageId);
    discussion.messagesCount--;
    await discussion.save();
    if (userMessage) {
      const messageDto: MessageDto = new MessageDto(userMessage);
  ...
  async getMessagesByDiscussionId(discussionId: string): Promise<MessageDto[] | null> {
    const discussion: HydratedDocument<Discussion> | null = await DiscussionModel.findById(discussionId);
    if (!discussion) throw ApiError.BadRequestError('сan`t find messages, discussion not found!');
    const messages = await MessageModel.find({ discussionId });
    if (messages) {
